...
async function discoverSeedFiles(): Promise<string[]> {
  ...
  async function scanDir(basePath: string) {
    ...
    for (const entry of entries) {
      ...
      const scanPaths = [
        modulePath,
        path.join(modulePath, "src", "seeds"),
        path.join(modulePath, "seeds"),
      ];
 ...
 * Any app/modules/<module>/*.seed.ts or app/features/<feature>/seeds/*.seed.ts
 * file can export seed functions as:
 * - default
